...
async def personal_account(message: Message):
    ...
    cur=conn.cursor()
    user_id=message.from_user.id
    cur.execute('SELECT user_name, user_interest, user_viewed_vids, user_test_result FROM users WHERE user_id = %s',
                (user_id,))
    rows=cur.fetchall()
    for row in rows:
        user_name, user_interest, user_viewed_vids, user_test_results = row
    if user_test_results == None:
        user_test_results='Вы еще не прошли тестирование'

    await message.answer(f'<b>Данные профиля</b>'
                             f'\nИмя: {user_name}'
                             f'\nОбласть интересов: {user_interest}'
                             f'\nПрохождение видео-курса: {user_viewed_vids}/16'
                             f'\nРезультат прохождения тестирования: {user_test_results}',
...
async def poll_answer(poll_answer: types.PollAnswer):
    ...
    if current_question_index <= 11:
        ...
        if 0 <= user_score <= 11:
            ...
            cur=conn.cursor()
            cur.execute('UPDATE users SET user_test_result = %s WHERE user_id = %s',
                ('низкий уровень компетенций', poll_answer.user.id))
            conn.commit()
            cur.close()
        elif 12 <= user_score <= 16:
            await bot.send_message(user_id,
                                   f'Спасибо за проходение опроса, ваш результат: уровень компетенций - ниже среднего. '
                                   f'\nСоветуем вам пройти видео-курс по команде \b"/study",'
                                   f' а затем попробуйте пройти акселерацию, она поможет вам улучшить свои навыки.')
            cur=conn.cursor()
            cur.execute('UPDATE users SET user_test_result = %s WHERE user_id = %s',
                        ('уровень компетенций - ниже среднего', poll_answer.user.id))
            conn.commit()
            cur.close()
            ...
            cur=conn.cursor()
            cur.execute('UPDATE users SET user_test_result = %s WHERE user_id = %s',
                        ('средний уровень компетенций', poll_answer.user.id))
            conn.commit()
            cur.close()
        else:
            await bot.send_message(user_id,
            ...
            cur=conn.cursor()
            cur.execute('UPDATE users SET user_test_result = %s WHERE user_id = %s',
                        ('высокий уровень компетенций', poll_answer.user.id))
            conn.commit()
            cur.close()
...
####Видео-курс####
videos=[
    {"title": "Приветственное видео", "url": "https://www.youtube.com/watch?v=NAKjRAY7NT4"},
    {"title": "Технологический суверенитет РФ", "url": "https://www.youtube.com/watch?v=Bu1mj_fXIUI"},
    {"title": "Инновационный продукт", "url": "https://www.youtube.com/watch?v=3OSaHvwXJwM"},
    {"title": "Методология развития клиентов (часть 1)", "url": "https://www.youtube.com/watch?v=NVBAhfjn2To"},
    {"title": "Методология развития клиентов (часть 2)", "url": "https://www.youtube.com/watch?v=SREJC1Zu8hQ"},
    {"title": "Методология JTBD", "url": "https://www.youtube.com/watch?v=MP1uusuhOTk"},
    {"title": "Анализ рынка", "url": "https://www.youtube.com/watch?v=iGHPY60i-DQ"},
    {"title": "Сегментация", "url": "https://www.youtube.com/watch?v=YXUTEeYuMNw"},
    {"title": "Бизнес-моделирование", "url": "https://www.youtube.com/watch?v=kxLZ6qQTI90"},
    {"title": "Маркетинг и PR (часть 1)", "url": "https://www.youtube.com/watch?v=STUPpVzgAws"},
    {"title": "Маркетинг и PR (часть 2)", "url": "https://www.youtube.com/watch?v=v42aAwtOZc0"},
    {"title": "Управление результатами ИД", "url": "https://www.youtube.com/watch?v=QvZagtcC-6E"},
    {"title": "Юнит-экономика (часть 1)", "url": "https://www.youtube.com/watch?v=TFEJ9jIgX1o"},
    {"title": "Юнит-экономика (часть 2)", "url": "https://www.youtube.com/watch?v=tx0Z5tzF4Is"},
    {"title": "Государственные инвестиции", "url": "https://www.youtube.com/watch?v=vENbtMdRNLM"},
    {"title": "Постакселерация", "url": "https://www.youtube.com/watch?v=tsg8cqaNAUU"}
]
...
async def send_video_menu(message: types.Message):
    keyboard=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="Назад", callback_data="back"),
                                                    InlineKeyboardButton(text="Выбрать",
                                                                         web_app=WebAppInfo(url=videos[current_video_index]["url"])),
                                                    InlineKeyboardButton(text="Вперед", callback_data="forward")],
                                                   [InlineKeyboardButton(text="Отметить просмотренное видео", callback_data="add_viewed_vid")]])
...
async def add_viewed_vid(callback: types.CallbackQuery):
    await callback.message.answer('Спасибо за просмотр. Примите во внимание, что за раз отмечается просмотр только одного видео')
    current_user_id = callback.from_user.id
    cur=conn.cursor()
    cur.execute('SELECT user_viewed_vids FROM users WHERE user_id=%s', (current_user_id,))
    rows=cur.fetchall()
    user_viewes = rows[0]
    user_viewes = user_viewes[0]
    user_viewes+=1
